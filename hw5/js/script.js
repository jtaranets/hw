...
const getTotalBalance = arr =>
  arr.reduce((acc, currVal) => (acc += currVal.balance), 0);
console.log(getTotalBalance(users));

const getUsersByFriend = (arr, name) =>
  arr.filter(item => item.friends.includes(name));
console.log(getUsersByFriend(users, "Briana Decker")); // [ 'Sharlene Bush', 'Sheree Anthony' ]
console.log(getUsersByFriend(users, "Goldie Gentry")); // [ 'Elma Head', 'Sheree Anthony' ]

const getAllSkills = function(arr) {
  const our = arr.map(item => item.skills).reduce((acc, item) => acc.concat(item)).sort();
  console.log(our);
  return our.reduce((acc, item) => acc.includes(item) ? acc : acc.concat(item), [])
  // return [...new Set(our)];
};
